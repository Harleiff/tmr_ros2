...
public:
    ...
    // Returns false if woken up for shutdown (empty sentinel)
    bool take(std::function<void()> &out);
};
...
public:
    void set_catch_data(const T &input);
    T get_catch_data();
};
...
template <typename T>
void MultiThreadCache<T>::set_catch_data(const T &input) {
